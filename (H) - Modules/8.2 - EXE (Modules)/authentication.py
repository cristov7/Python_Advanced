...
def render_entry():
    register_btn = Button(root, text="Register", bg="green", fg="white", borderwidth=0, width=20, height=3, command=register)
...
def check_registration(info):
    for el in info.values():
        if not el.strip():
            frame.create_text(300, 300, text="We are missing some information!", fill="red", tag="error")
            return False
    frame.delete("error")
    info_data = get_users_data()
    for record in info_data:
        if record["username"] == info["username"]:
            frame.create_text(300, 300, text="User with this username already exists!", fill="red", tag="error")
            return False
    frame.delete("error")
    return True
...
def check_if_login_is_fulfilled(event):
    info = {"username": username_box.get(),
